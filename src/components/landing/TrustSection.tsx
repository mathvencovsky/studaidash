  ...
  return (
    <SectionWrapper id="privacidade-controle" variant="tint" tabIndex={-1}>
        ...
        <Card className="border-2 border-primary/20 shadow-lg bg-gradient-to-br from-card to-primary/5">
          <CardContent className="p-6">
            <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
              <span className="w-6 h-6 rounded-full bg-accent-warm/15 flex items-center justify-center">
                <span className="text-accent-warm text-xs">30s</span>
              </span>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
        {trustCards.map((card) => (
          <Card key={card.title} className="bg-card border-2 hover:border-primary/30 transition-all hover:shadow-lg group">
      ...
      <div className="flex flex-wrap justify-center gap-4 mt-8">
        <Link
          to="/privacidade"
          className="inline-flex items-center gap-1.5 text-sm text-primary font-medium hover:underline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded"
        >
